import React from "react";
import { NavLink } from "react-router-dom";
import logofd from "../component/Assets/logofd.png";
function Navbaradmin() {
    return (
        <nav class="flex px-4 border-b md:shadow-lg items-center relative bg-white" style={{ position: 'fixed', top: '0', width: '100%', zIndex: '100' }}>
            <div class="flex items-center">
            <img src={logofd } alt="" height="80" width="100"/>
                <span className="text-lg font-bold m1-2" style={{ color:"orange" ,fontSize: "28px"}}>Tastify</span>
            </div>
            <ul class="md:px-2 ml-auto md:flex md:space-x-2 absolute md:relative top-full left-0 right-0">
                <NavLink to={"/adminhome"}>
                    <li>
                        <a href="#" class="flex md:inline-flex p-4 items-center hover:bg-orange-300">
                            <span>Users</span>
                        </a>
                    </li>
                </NavLink>
                <NavLink to={"/adminmenu"}>
                    <li>
                        <a href="#" class="flex md:inline-flex p-4 items-center hover:bg-orange-300">
                            <span>Menu</span>
                        </a>
                    </li>
                </NavLink>

                <NavLink to={"/feedback"}>
                    <li>
                        <a href="#" class="flex md:inline-flex p-4 items-center hover:bg-orange-300">
                            <span>Feedback</span>
                        </a>
                    </li>
                </NavLink>
                <NavLink to={"/adminlogout"}>
                    <li>
                        <a href="#" class="flex md:inline-flex p-4 items-center hover:bg-orange-300">
                            <span>Logout</span>
                        </a>
                    </li>
                </NavLink>
            </ul>
            <div class="ml-auto md:hidden text-gray-500 cursor-pointer">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 fill-current" viewBox="0 0 24 24"><path d="M24 20.188l-8.315-8.209 8.2-8.282-3.697-3.697-8.212 8.318-8.31-8.203-3.666 3.666 8.321 8.24-8.206 8.313 3.666 3.666 8.237-8.318 8.285 8.203z" /></svg>
            </div>
        </nav>
    );
}

export default Navbaradmin;